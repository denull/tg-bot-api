...
Выбрасываемые ошибки являются экземплярами класса `TelegramBotAPI.TelegramError` (который наследуется от `Error`). Он содержит следующие поля:

- `message`: текстовое описание ошибки, соответствует полю `description` в ответе сервера;
- `code`: числовой код ошибки, соответствует полю `error_code` в ответе сервера;
- `parameters`: дополнительные [параметры ошибки](https://core.telegram.org/bots/api#responseparameters), возвращённые сервером;
- `request`: объект с полями `method`, `params`, `attempt` и `retryAttempts`, описывающий запрос, вызвавший ошибку (`attempt` — номер попытки, начиная с 0).
...
Для глобальной обработки ошибок в ответах API (например, для логирования), установите вашу функцию-обработчик в поле `onError` на вашем экземпляре `TelegramBotAPI`. Она будет вызываться перед выбрасыванием каждой ошибки с единственным аргументом — самой ошибкой.
...
Второй способ — указать эти значения в полях `name` и `type` непосредственно на загружаемом объекте. Кроме того, если экземпляр класса `Stream` содержит поле `path`, имя файла будет извлечено из пути к нему.
...
## Валидация initData для мини-приложений

Для удобства при разработке мини-приложений доступен метод `verifyInitData`. Он принимает в качестве параметра строковое значение поля `initData` (см. [документацию по мини-приложениям](https://core.telegram.org/bots/webapps#initializing-mini-apps)) и проверяет что у него корректно рассчитан хэш. Если хэш корректен, метод возвращает декодированный объект, в противном случае возвращается `null`. Вторым параметром можно передать максимально допустимый срок давности в секундах (по умолчанию 30 дней; чтобы отключить проверку передайте `Infinity` или любое falsy значение).

Кроме того доступны два вспомогательных метода (обычно их не нужно использовать напрямую):
* `computeInitDataHash` — рассчитать корректный хэш по объекту `initData`;
* `signInitData` — преобразовать объект в строку и подписать корректным хэшом (может быть полезно при тестировании).